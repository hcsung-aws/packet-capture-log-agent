...
- [x] 3: 프로토콜 JSON 자동 생성 — AgentCore 전체 구현
  - [x] 3a: PoC (로컬 Bedrock 직접 호출, 5 Phase 파이프라인)
  - [x] 3b: AWS 인프라 (Terraform: Lambda + Step Functions + S3 + API Gateway)
  - [x] 3c: CLI 클라이언트 (generate, status, download)
  - [x] 3d: 웹 프론트엔드 (Flask + 바닐라 HTML/JS)
  - [x] 3e: Windows 배포 바이너리 (PyInstaller → exe + zip)
- [x] 4: field_variants — 같은 액션 반복 시 필드값 변화 추적 + 랜덤 선택 (이동 방향 고정 문제 해결)
- [x] 5: analyze_all.ps1 — 다중 로그 일괄 분석 스크립트
- [x] 6: 문서 최신화 — PROJECT-OVERVIEW, FILE-STRUCTURE, project-context, agentcore-architecture, README

## Progress

### Completed
- 1+2: 이전 세션에서 완료 (semantics, string_prefixed, conditional)
- 3a: PoC 전체 파이프라인 구현 + E2E 검증 완료 (~8분, 53 packets, 참조 대비 완벽 일치)
- 3b: Terraform 배포 완료 (27 리소스). E2E 검증 (~7분, 동일 결과)
  - Lambda Layer python/ prefix 이슈 수정
  - Discovery에서 relevant_files 반환 추가
  - SFN Pass state 수정
- 3c: CLI 클라이언트 (generate/status/download)
- 3d: 웹 프론트엔드 (Flask + 바닐라 HTML/JS, 비동기 폴링)
- 3e: PyInstaller로 Windows exe 빌드 (CLI 28MB + Web 29MB → zip 55MB)
- 4: field_variants 구현 (ActionCatalogBuilder + ActionExecutor)
  - 근본 원인: 같은 액션 반복 시 첫 번째 필드값만 저장, 나머지는 RepeatCount++만
  - 해결: 필드값 변화 시 variants에 누적, 실행 시 랜덤 선택
- 5: analyze_all.ps1 (디렉토리/파일 목록 일괄 분석)
- 6: 문서 전체 최신화 + agentcore-architecture.md 신규 생성
...
- agent-core/lambda/: common/(llm_client, s3_helper, prompt_loader, prompts/), layer_pkg/, discovery/, analysis/, merge/, generation/, validation/, orchestrator/
- agent-core/terraform/: main.tf, variables.tf, outputs.tf
- agent-core/client/: cli.py, app.py, web/index.html, requirements.txt
- PacketCaptureAgent/: ActionCatalogBuilder.cs (field_variants), ActionExecutor.cs (랜덤 선택), analyze_all.ps1
- 문서: PROJECT-OVERVIEW.md, FILE-STRUCTURE.md, README.md, context_rule/project-context.md, context_rule/agentcore-architecture.md, context_rule/INDEX.md, auto_notes/NOTES.md
...
- 같은 액션 반복 시 필드값이 다르면 "가변 필드"로 추적해야 함 — 첫 번째 값만 저장하면 이동 방향 등이 고정됨
- 세션 로그/핸드오프에 설계 논의 상세 내용 포함 필요 — 요약이 과도하면 다음 세션에서 컨텍스트 유실

## Context Window Status
~45%

## Next Steps
- 모델 비교 (Haiku, Sonnet 3.7 vs Sonnet 4 — 비용 대비 효율)
- field_variants 적용 후 BT 재생성 + 이동 방향 다양화 검증
